...
from filler import get_response
from filler_checker import similarity
...
def bigger_data():  
    for intent in data["intents"]:  # each tag intents
        ...
        for j in extracted:  # for each string in responses
            ...
            results = []
            try:
                result = get_response(j, 3, 6)    # Result is a list of string of datafiller result
            except IndexError:
        ...
        with open('school_filled2', 'w') as file:
